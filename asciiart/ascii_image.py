#! /usr/bin/env python3
"""
Converts images or videos to ASCII in the terminal.

Adapted by halloleo (http://github.com/halloleo)
        from Ali Shazly (https://github.com/AliShazly/ascii-py)
"""
import argparse
...
def print_image(img, resolution, greyscale=False, bg_color=False, fg_color=True, reverse=False):
    if greyscale:
        bg_color = False
        fg_color = False
    ...
    chars = CHARS if not reverse else list(reversed(CHARS))
...
def main():
    ...
    parser.add_argument('-v', '--reverse', action='store_true',
                        help='Reverse the intensity scheme')
    ...
    if args.json:
        ...
        print_image(img, args.resolution, greyscale, background, foreground, args.reverse)
